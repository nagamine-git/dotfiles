...
The repo is a chezmoi source tree for three machines: one macOS host (`darwin` branches) and two Arch-family Linux hosts — mainly CachyOS, EndeavourOS-compatible (`am5-itx` = default branch, `fp7-e14` = hostname branch). Files prefixed with `dot_` mirror home dotfiles (`dot_zshrc`, `dot_tmux.conf`), while `dot_local/` shadows `~/.local` assets such as scripts in `dot_local/bin/`. Sensitive data and host overrides live under `private_*` (for example, `private_dot_config/hypr/`). Machine-wide configs belong in `etc/`, and reusable values go in `.chezmoidata/`. Keep helper scripts in `run_onchange_*.sh`, updating `run_onchange_setup.sh` when automation is required.
...
Shell scripts target Bash; start them with `#!/usr/bin/env bash` and `set -eu`. Prefer small idempotent functions, explicit `case` statements for OS-specific branches, and consistent two-space indentation. Keep comments brief and functional. Follow existing naming: `dot_config/<app>/` for config directories, `private_*` for secrets. Host-specific behavior belongs in chezmoi template conditionals (`{{ if eq .chezmoi.hostname "fp7-e14" }}` etc.) — do not hardcode hostnames in plain scripts, unit files, or `pkglist.txt`.

## Testing Guidelines
Automated checks exist in two layers: CI (`.github/workflows/chezmoi.yml`: template render / dry-run apply, gitleaks, shellcheck `severity: error`) runs on every push/PR to main, and pre-commit hooks (`.pre-commit-config.yaml`: gitleaks, shellcheck, whitespace/YAML checks) run locally after `pre-commit install` — run that once per checkout. Additionally rely on `chezmoi diff` and `chezmoi apply --dry-run` to confirm template output. After editing shell helpers, run the lint commands above and execute scripts in a disposable shell when risky. Capture before/after snapshots or screenshots when desktop tooling is affected.
